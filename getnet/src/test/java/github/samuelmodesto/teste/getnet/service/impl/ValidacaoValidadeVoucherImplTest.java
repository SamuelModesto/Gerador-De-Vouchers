...
@SpringBootTest
class ValidacaoValidadeVoucherImplTest {
    ...
    @InjectMocks
    private ValidacaoValidadeVoucherImpl service;
    ...
    @Test
    void quandoADataDeValidadeEstiverVencidaDeveRetornarExcecao() {
        try {
            service.validarVoucher(optionalDestinatario, codigo);
        } catch (Exception ex) {
            assertEquals(ValidacaoException.class, ex.getClass());
            assertEquals("Voucher fora da validade", ex.getMessage());
        }
    }
